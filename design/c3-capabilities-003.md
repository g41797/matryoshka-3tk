...
than carried: *For 3TK-5*, which instructed a stage that ran in 2026-08, and
*What this study rules on*, which resolved eleven conflicts between seven
drafts that no longer exist. Nothing in either was still true and unrecorded
elsewhere — Q4 already ends by saying it reopens a question the drafts thought
closed, and the `anylist` collision is in *Verified spellings* below. **Q13 is
new**, folded in from `3tk-build-dist.md` and rebuilt from `c3c --help` on
0.8.3 rather than carried, because the document it came from was unmeasured
prose.

The capability questionnaire is **Part 21** of the shared specification —
`matryoshka-specification-001.md` when this study was written, and the Part
number is a historical marker here, not a live link.
...
- **Re-measured 2026-09-09 by 3TK-73, against the installed toolchain: 0.8.3,
  git `1d155ee`, LLVM 22.1.8 — unmoved.** Every answer below was measured on
  the compiler that is installed today.
...
| Q13 | Packaging and distribution | **yes, early** — `.c3l`, and no registry |

Q13 is not one of Part 21's twelve. It was folded in by 3TK-73 and is marked as
what it is: a language capability the port depends on and the questionnaire
never asked about.
...
**This closes the question of whether `typeid` satisfies Part 5.1.** Four of
the 2026-08 drafts assumed it; the fifth flagged it as the dangerous area. The four were right, and the assumption is
...
**This closes the `inline`-or-offset question.** `3tk-polyhelper.md` assumed offset
...
**This closes the hooks question — interface, not struct** — on the mechanism. It does not close
...
**The language imposes nothing here, and the port has since chosen.** Part 13.4
leaves the application-outer half to the port and Part 20 decision 2 lists it;
Part 13.1 settles the two containers. 3tk ruled both ways down: **a container
keeps its allocator for life and no release call takes one, and an application
outer keeps its allocator in the outer, never in the inner.** An early draft
wrote `destroy(Allocator, slot)`; that was a choice C3 permitted and the port
declined. **Recorded here as what C3 allows, not as a recommendation** — the
ruling was 3tk's, for 3tk.
...
**On what the Slot is, and what it is called.** The probes make the shape plain.
...
**On typed handles, and what they cost.** The second negative probe is the cost
...
# Q13 — Packaging and distribution

**Yes, and it is early.** Verified on 0.8.3, 2026-09-09. **Not one of Part
21's twelve** — folded in by 3TK-73 from `3tk-build-dist.md`, and rebuilt from
the compiler rather than carried, because that document was unmeasured prose
written against an earlier understanding.

**There is no package manager and no registry.** C3 has no `build.zig` +
registry and no module proxy. What it has is a distribution unit, a search
path, and a fetch command for one curated collection.

**The unit is a `.c3l`.** A directory whose name ends `.c3l`, holding a
`manifest.json` and sources, optionally with prebuilt libraries per target.
`c3c init-lib demo` creates it:

```
demo.c3l/
├── manifest.json
├── demo.c3i                    ← the interface file
├── LICENSE
└── linked-libs/
    ├── linux-x64/
    ├── macos-aarch64/
    ├── wasm32/
    └── ... one directory per supported target
```

**The manifest names what the library provides and what each target links.**
The generated one, verbatim in shape:
...
**Two things about that file are worth writing down**, because both surprise a
reader who expects JSON: **it carries `//` comments**, and **the generated one
ends its `targets` object with a trailing comma**. It is JSON5-shaped, and a
strict JSON parser rejects what the compiler itself wrote.

**`sources` is commented out by default**, so a fresh `.c3l` compiles nothing
until the line is uncommented or the sources are placed where the compiler
looks.

**The consumer side is two flags.**

```
c3c compile --libdir ./lib --lib demo myapp.c3
```

`--libdir <dir>` adds a directory to the `.c3l` search path; `--lib <name>`
adds one library from it to the compilation. Both are on `c3c --help` for
0.8.3.

**The rest of the surface, as the compiler states it.**

| what | spelling |
|---|---|
| a project | `c3c init <name>`, `--template exe \| static-lib \| dynamic-lib \| <path>` |
| a library | `c3c init-lib <name>` |
| a library with no project | `c3c static-lib <files>`, `c3c dynamic-lib <files>` |
| fetch from the curated collection | `c3c vendor-fetch <library> ...` |
| C headers for a library | generated by default; `--no-headers` off, `--header-output <dir>` elsewhere |

**What this costs the port.** Nothing structural — 3tk builds through its own
`project.json` and its scripts, and has never shipped a `.c3l`. It is recorded
because *how does this port get distributed* is a question every port's owner
asks once, and the honest answer for C3 today is: a directory you copy, a
manifest a strict parser will not read, and no registry to publish to.
