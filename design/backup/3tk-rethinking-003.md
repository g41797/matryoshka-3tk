...
**Written 2026-09-06, in 3TK-61. Sections 10-12 added the same day, on the
owner's line: the shape of `OuterHelper`. Section 13 and MS-6..MS-8 added the
same day, on the owner's second line: how the internal macros stop being
client-reachable, section 14 on how the outer's fields are discovered, and
section 15 on how the whole thing is used — by a user, and by the mailbox and
the pool. Sections 10-15 are a proposal; MS-6..MS-9 are measured.**

**Supersedes [3tk-rethinking-002.md](backup/3tk-rethinking-002.md), which
supersedes `001`.**
...
what section 13 spends.

### MS-7 — a generic instantiated on a `@private` type, measured 2026-09-06

Section 13 turns on a shape that M1-M4 never tested, so it was probed before
being proposed: **module `mtk::inner <Outer>` declaring `OuterHelper`, aliased
...
| `Named` | `Allocator a;` | *"nothing to set"* — **silently**, see 14.3 |
...
> as a member with **zero user sites**, for the outer built by hand.
>
> **Corrected by 15.5:** it has zero *user* sites and **two mtk-internal
> ones**, `mailbox.c3:95` and `pool.c3:193`. It was written here as the member
> this proposal was least sure of — **Q-6** — and 15.5 answers it: `init`
> stays, and mtk is its consumer.
...
**This was written as genuinely close.** 15.5 makes it not close: `init` has
two real callers inside mtk itself, and a member with a caller is not a member
that earns nothing. **V-5a, and no longer narrowly.**
...
**`V-6` is not on this graph.** It was written after sections 10-12, it lives
in section 13, and it depends on none of `MS-4`, `MS-5`, `V-1`..`V-5`. It
depends only on 3TK-63 having run, and it lands inside 3TK-65.
...
6. **Q-6 — does `init` stay a member?** V-5a keeps it with zero user sites and
   ...
   the work. **Answered by 15.5**, which found the two sites the question was
   missing — `mailbox.c3:95` and `pool.c3:193`. It stands only if the owner
...
10. **Q-10 — is `RT-3`'s module name spent?** Tier A needs `helper.c3` to
    declare `module mtk::inner;`. The file, the doc block and the chapter are
    untouched; the user's alias line reads `mtk::inner::OF{Msg}` instead of
    `mtk::helper::OF{Msg}`. **One word, once per Outer type.** Yes or no —
    a stage may not assume this (13.2).

11. **Q-11 — `PL-7`, and it is no longer optional.** Do the mailbox and the
    pool reach the crossings **through `OuterHelper`**, on two aliases of
    their own for `_Mbox` and `_Pool`? If no, the crossings stay public
    forever and Tier A does not happen. There is no third road (13.3).

12. **Q-12 — how far does the merge go: `V-6a`, `V-6b` or `V-6c`?** `V-6a`
    brings `queue.c3` in and makes the chain mutations unreachable too, at
    the cost of the module name `mtk::queue` — **which nothing outside
    `src/` spells and nothing anywhere imports.** `V-6c` is the status quo
    and should be chosen on purpose, not by default (13.6).

13. **Q-13 — does `run-builds.sh` assert the surface?** 13.5 ends with a
    short list — `mtk`, `mtk::mailbox`, `mtk::pool`, `xtn` — of every module
    a user may name. It is checkable. Should the check exist, and does it
    belong to 3TK-65 or to 3TK-66 with the books?

14. **Q-14 — is the allocator keyed on the name `alloc`, and does the
    refusal come with it?** Keying on the name lets an outer keep two
    allocators, which `RT-12` and `RT-13` already imply it should be allowed
    to. **The refusal is the half that matters:** without it, an outer with
    `Allocator a;` is silently unmanaged and its allocator is never set.
    Two answers, and the second is not automatic (14.1, 14.2).

15. **Q-15 — the inner: the name `inner`, and then `V-7a` or `V-7b`?** The
    name costs zero example changes and buys a message that names the fix.
    `V-7a` ignores any other `Inner`; `V-7b` puts the outer on several chains
    and spends the surface section 10 just finished shrinking. **`V-7a` does
    not foreclose `V-7b`** — the default parameter serves both (14.3, 14.4).

**Not asked here, deliberately:** nothing. `PL-7` was left alone by sections
10-12 and is asked as `Q-11`, because section 13 found that Tier A cannot have
it both ways.

---

# 13. Making the internals unreachable — a proposal

**The owner's question:** after 3TK-63, `inner.c3` holds a lot of macros used
only by the helper, and possibly by the mailbox and the pool. **How are they
made private to client code and still useful inside mtk?**

`MS-6` is the constraint and it is unforgiving: **`@private` means the module
and nothing else** — not a submodule, not the parent. There is no package
visibility in C3. So the only instrument available is *which symbols share a
module*, and the only fact that makes this workable is that **a C3 module may
span many files**. Nothing below merges a file into another file; every file
keeps its name, its doc block and its chapter. Only the `module` line changes.

## 13.1 The three tiers

Every symbol that will live in `inner.c3` after 3TK-63 falls into one of three
tiers, and the tier is decided by **who calls it from outside**.

**Tier A — the crossings.** The seventeen crossing macros plus `inner_offset`
and `required_alloc_offset`. After 3TK-65 their only consumer is `OuterHelper`.
If `helper.c3` declares `module mtk::inner;` instead of `module mtk::helper;`
— one module, two files — every one of them takes `@private` and **no client
module can name one.** `MS-7` measured exactly this shape and it holds.

**Tier B — the chain mutations.** `Inner.repoint_to`, `Inner.points_to`,
`is_linked`, `reset`. These are the container's instruments. `HS-7` already
says the helper must not expose them, so the helper cannot carry them across
the boundary for anyone. Their callers are `queue.c3`, `mailbox.c3`, `pool.c3`
— see 13.3, which moves most of this tier into A.

**Tier C — the Slot's own operations.** `Slot.fill`, `Slot.take`, `Slot.peek`.
The mailbox and the pool call them at a dozen sites across a module line and
always will. **These stay public.** That is not a defeat: `Slot` is a
user-facing type, and three methods held by a doc line is a promise of a
different size than twenty-odd macros held by the same line.

**Everything used by exactly one file takes `@local`, not `@private`.** It is
free and strictly tighter. One pass at the end of 3TK-63.

## 13.2 `RT-3`'s module name is the price of Tier A

`RT-3` named the module `mtk::helper`. Tier A spends that name: the file stays
`helper.c3`, the module becomes `mtk::inner`, and the user's alias line reads

> `alias MSG = mtk::inner::OF{Msg};`

instead of `mtk::helper::OF`. **One word, in the one line per Outer type that
`HR-1` already budgets** — and that line is the only place a user names the
module at all, because `HS-3`..`HS-5` are members reached through `MSG`.
**This is the owner's to re-rule; a stage may not assume it (`Q-10`).**

## 13.3 The mailbox and the pool are already clients of the helper

This is the finding that changes the shape, and it is not speculation.
**`_Mbox` and `_Pool` are Outers.** They embed an `Inner` and they cross the
same boundary user code crosses, at six sites:

| site | call |
|---|---|
| `mailbox.c3:73` | `mtk::helper::to_inner((_Mbox*)p)` |
| `mailbox.c3:78` | `mtk::helper::from_inner(inner, _Mbox)` |
| `mailbox.c3:95` | `mtk::helper::init(mb)` |
| `pool.c3:150` | `mtk::helper::to_inner((_Pool*)p)` |
| `pool.c3:155` | `mtk::helper::from_inner(inner, _Pool)` |
| `pool.c3:193` | `mtk::helper::init(p)` |

**So `PL-7` stops being a matter of taste and becomes a precondition.** If the
crossings go `@private`, those six sites stop compiling. The mailbox and the
pool cannot join the module either: `mtk::mailbox::create` and
`mtk::pool::create` collide the moment they share one, and both names are
user-facing — 28 and 15 example sites. There are exactly two roads:

1. **the mailbox and the pool reach the crossings through `OuterHelper`**, on
   two aliases of their own, for `_Mbox` and `_Pool`; or
2. **the crossings stay public forever.**

`RT-11` left this "deliberately undecided". It cannot stay undecided and also
have Tier A. Road 1 is the recommendation, and it is not a concession —
**it is mtk holding its own types to `HR-1`.** `MS-7`'s probe was written on
`_Mbox` for exactly this reason, and it passed with `_Mbox` `@private`.

## 13.4 `queue.c3` joins for free, and that is the larger prize

**Nothing outside `src/` spells `queue::`, and nothing anywhere imports
`mtk::queue`.** The examples receive `InnerQueue` as a *type*, through
`import mtk` and `import mtk::mailbox`. **The module name is invisible to
users.** So `queue.c3` may declare `module mtk::inner;` at zero user cost —
the same one-module-many-files move, and the same move 3TK-62 already makes on
the stack — and then **Tier B goes `@private` too.**

That is the difference between a documentation promise about the chain
mutations and a compiler-enforced one.

## 13.5 The surface that results

**`module mtk::inner`** = `inner.c3` + `helper.c3` + `queue.c3`. Public from
it: `Inner`, `Slot`, `Slot.is_empty`/`is_full`/`fill`/`take`/`peek`,
`InnerQueue`, `OuterHelper` and its members. **Everything else `@private` or
`@local`.**

**The modules a user may name:** `mtk`, `mtk::mailbox`, `mtk::pool`, `xtn`.
That is the whole surface, and it is checkable — `run-builds.sh` can assert it.

## 13.6 The variant

**`V-6` — how far the merge goes.**

| | shape | Tier A private | Tier B private | cost |
|---|---|---|---|---|
| **V-6a** | `inner.c3` + `helper.c3` + `queue.c3` are one module | yes | **yes** | `RT-3`'s name; `PL-7` ruled road 1; `queue.c3`'s module line |
| **V-6b** | `inner.c3` + `helper.c3` only | yes | no | `RT-3`'s name; `PL-7` ruled road 1 |
| **V-6c** | nothing merges | no | no | none — and nothing is enforced |

**Recommendation: `V-6a`.** It is the only one that ends with the chain
mutations unreachable, it costs one module line in a file no user names, and
`MS-6`/`MS-7` have measured every mechanism it uses. `V-6b` is the honest
fallback if the owner keeps `mtk::queue` as a name users may reach. **`V-6c` is
the status quo**, and it is a real option — it costs nothing and enforces
nothing — but it should be chosen on purpose rather than by default.

**Dependencies.** `V-6` needs 3TK-63 to have happened (the macros must be in
`inner.c3` before they can be made private) and 3TK-65 to be happening (the
helper must exist before the mailbox and the pool can route through it). It
therefore **lands in 3TK-65, not in its own stage**, and it does not reorder
3TK-62..66. `V-6` also **decides `PL-7`**, which `V-2` and section 11 left
alone. Nothing in `V-6` depends on `MS-4` or `MS-5`.

**What `V-6` does not touch.** The example files. Not one of the 200 measured
sites changes: users already reach nothing in Tier A or Tier B. **A change
this size that no user file notices is the argument for it.**

---

# 14. How the outer's fields are discovered — a proposal

**The owner's question, in two halves.** Is the `Inner` found by *type*, or by
the *name* `inner`, and may an outer carry several? And: may an outer carry
several `Allocator`s, or none, with only `Allocator alloc` being the field that
is set? `MS-9` measured both. Both work. **They are not the same decision, and
they carry different risk.**

Today both discoveries key on the **type** and assert `$seen == 1`:
`inner_offset` at `inner.c3:168`, `required_alloc_offset` at `inner.c3:195`.

## 14.1 The allocator — the owner is right, and it removes a real limitation

`RT-12` and `RT-13` make the `Allocator` field **optional**, and say plainly
that it exists **for the outer's own later allocations**. But an outer that
keeps two — one for itself, one for a sub-buffer it owns — **cannot be created
today**, because the type-keyed scan rejects the second. That is not a policy,
it is a leftover from when the field meant "this type is managed".

**Keying on the name `alloc` removes it**, and "found twice" stops being an
error class at all. `MS-9`'s `Two` case is exactly this shape and it resolves
cleanly.

**This amends `RT-13`**, which reads *"not found is an answer, found twice is
still an error"*. Under the name rule, **found twice is also an answer.**

## 14.2 The hazard: the reclassification is silent

`RT-13` makes *no allocator* legal. So a field named anything but `alloc` does
not fail to compile — **it silently reclassifies the outer as unmanaged**, and
the allocator it does carry is never set. Two sites in the tree are exactly
that shape:

| site | field | under the name rule |
|---|---|---|
| `examples/035-fan_in.c3:22` | `Allocator a;` | silently unmanaged |
| `test/t_alloc.c3:36` | **`Allocator inner;`** | silently unmanaged |

`t_alloc.c3` exists **to prove the field name does not matter**. Under this
rule it asserts the opposite and must be rewritten, not adjusted. And
`Allocator inner;` is a collision worth keeping in view: under both new rules
that one field is **neither the allocator nor the inner**.

**The mitigation, and it should be part of the ruling rather than a follow-up:
`xtn::create` refuses at compile time when the type carries an `Allocator`
field but none named `alloc`** — *"you have an Allocator, but not `Allocator
alloc;`"*. It converts the only silent failure into a loud one, it costs a
second `@const` predicate, and it cannot fire on an outer that carries no
allocator at all, which stays legal. **`Q-14`.**

## 14.3 The inner — the name costs nothing and buys a better message

**Zero example files change**: 15 of 15 already write `Inner inner;`. What the
rule buys is the message. Today an outer that forgot the field is told *"type X
has no Inner field"*; with the name rule it is told **"type X must declare
exactly one `Inner inner;`"** — the fix, not the symptom. And the *two inners*
error class disappears, because a second `Inner` under another name is simply
not seen.

**This amends `Part 4.4`**, which rules exactly one `Inner` per outer.

## 14.4 "Several inners" is two features, not one — `V-7`

| | rule | cost |
|---|---|---|
| **V-7a** | the `Inner` named `inner` is the toolkit's; **any other `Inner` is yours and is ignored** | nothing |
| **V-7b** | an outer may sit on **several chains at once**, one per named `Inner` | the field name threads through **every** crossing; **one identity stamp per `Inner`**; `is_linked` and `reset` multiply; the user needs a second alias per type (**breaching `HR-1`**) or a name argument on every member |

`MS-9` shows the *mechanism* for `V-7b` works — the probe selected `other` and
`more` by name. **The cost is not the mechanism, it is the surface**, and
section 10 spent its whole argument getting that surface down to nine members.

**Recommendation: `V-7a`, and the reason it is safe is the default parameter.**
One macro, `$name = "inner"`, serves both shapes. **Ruling `V-7a` today
forecloses nothing** — `V-7b` stays reachable, unruled, at no cost, the day a
real user wants an outer on two chains. **`Q-15`.**

## 14.5 What this touches

Not the user surface: **no example file changes for the inner half**, and one
for the allocator half. The blast radius is the tests, and it is loud:

- `negative/nocompile_two_inners.c3` **stops failing** — it becomes a positive
  test, or a new negative one that omits `Inner inner;` instead.
- `negative/nocompile_managed_two_allocators.c3` **stops failing** — same
  choice, and 3TK-64 is already rewriting its neighbour
  `nocompile_managed_no_allocator` for `RT-13`.
- `test/t_alloc.c3` **inverts**, per 14.2.
- `run-builds.sh`'s negative-program checks change with them.

**Where it lands.** The allocator half belongs to **3TK-64**, which already
moves `required_alloc_offset` into `xtn` and already rewrites that negative
program. The inner half belongs to **3TK-63**, which already touches
`inner_offset`'s fourteen call sites. **Neither needs a stage of its own**, and
neither reorders 3TK-62..66.

**`_Mbox` and `_Pool` are unaffected.** They carry `Allocator _alloc;` but do
not go through `required_alloc_offset`; they allocate directly. Worth writing
down only so a later reader does not rename them believing the rule reaches
them.

---

# 15. How it is used — the user, the mailbox, the pool

**Sections 10-14 propose a shape. This one spends it**, against a file that
exists: `examples/006-defer_put_early.c3`, 57 lines, which exercises the pool,
the hooks, a crossing and a `defer` at once. **Nothing here is new; it is
sections 10-14 written out at a call site**, because a shape that reads well as
a table can still read badly as code.

Two audiences, and the finding at the end is that **they end up with the same
surface**.

## 15.1 The user: one alias, ten members

`Holder` is declared once, in `examples/outers.c3:14`:

```c3
struct Holder { int id; Inner inner; Allocator alloc; }
```

Worth noticing before anything else: **it already satisfies both of section
14's name rules** — `Inner inner;` and `Allocator alloc;` — as do 15 of 15
outers in the tree. Section 14 renames nothing here.

Beside it, once per outer type, `HR-1`'s entire ceremony:

```c3
alias HOLDER = xtn::OF{Holder};
```

**One alias reaches all ten members**, because `V-2b` has `XtnHelper` embed
`OuterHelper` `inline`. There is no second alias for the allocating half —
that is the whole point of `V-2b`, and it is what `MS-5` is owed to confirm.

Every helper site in the example then collapses onto it:

| today | under the proposal |
|---|---|
| `mtk::managed::create(Holder, self.alloc, slot)` | `HOLDER.create(self.alloc, slot)` |
| `mtk::managed::release(Holder, &s)` | `HOLDER.release(self.alloc, &s)` |
| `s.must(Holder).id = 1` | `HOLDER.must(&s).id = 1` |

**The type name stops being an argument and becomes the receiver.** The imports
thin with it: `import mtk::managed` becomes `import xtn`, and **a user imports
`mtk::helper` never** — under `V-6a` no module goes by that name.

`release` gaining the allocator (`RT-12`) is visible in the second row. This
example pays nothing for it — `on_close` already holds `self.alloc` — but that
is luck, not a rule, and the 78 `release` sites will not all be so lucky.

## 15.2 The dispatch site

`RT-17` blesses the hand-written `switch`, and this is the site that justifies
keeping `owns` on its single measured use (`HS-4`):
...
while (Inner* inner = q.pop_front())
{
    switch
    {
        case MSG.owns(inner):  handle(MSG.must_from(inner));
        case ACK.owns(inner):  handle(ACK.must_from(inner));
        default:               drop(inner);
    }
}
```

Two members, one predicate, no dispatch construct, and **no type name in the
call** — the alias carries it.

## 15.3 The user who builds an outer by hand

`HS-10` is what makes this work without an `init` the user must remember: the
first `HOLDER.inner(&h)` stamps the identity, writing `.type` only, so it is
safe whether or not the outer is already linked. **`HR-5` is discharged by the
crossing the user was going to make anyway.**

## 15.4 The mailbox and the pool use it identically

Because **`_Mbox` and `_Pool` are outers** — 13.3's finding. One alias each,
and private, per `MS-7`'s residue that an alias is public unless marked:

```c3
module mtk::mailbox;
struct _Mbox @private { ... Inner inner; ... }
alias MBOX @private = mtk::inner::OF{_Mbox};
```

The six sites of 13.3 become:

| site | today | under the proposal |
|---|---|---|
| `mailbox.c3:73` | `mtk::helper::to_inner((_Mbox*)p)` | `MBOX.inner((_Mbox*)p)` |
| `mailbox.c3:78` | `mtk::helper::from_inner(inner, _Mbox)` | `MBOX.from(inner)` |
| `mailbox.c3:95` | `mtk::helper::init(mb)` | `MBOX.init(mb)` |
| `pool.c3:150` | `mtk::helper::to_inner((_Pool*)p)` | `POOL.inner((_Pool*)p)` |
| `pool.c3:155` | `mtk::helper::from_inner(inner, _Pool)` | `POOL.from(inner)` |
| `pool.c3:193` | `mtk::helper::init(p)` | `POOL.init(p)` |

**After this, no module outside `mtk::inner` names a crossing macro.** That is
precisely the precondition Tier A needs (13.3), and it is reached by rewriting
six lines.

**What the containers still reach for raw** is Tier C — `Slot.fill`,
`Slot.take`, `Slot.peek` — a dozen sites, public by necessity (13.1). And one
site stays raw for a different reason: `pool.c3:354`,
`slot.peek().link.type == want`, where `want` is a **runtime** `typeid` handed
in by the caller. `owns` is instantiated on a compile-time type and cannot
express that. **It should not be made to try.**

## 15.5 This answers `Q-6`, and `HS-5` is corrected

`HS-5` called `init` *"the one member this proposal is least sure of"*, on
`MS-1`'s evidence that users call it **zero** times. **That evidence was
incomplete.** `init` has zero user sites and **two mtk-internal ones** —
`mailbox.c3:95` and `pool.c3:193` — because the mailbox and the pool build
their own outers by hand rather than through `xtn::create`.

**`init` stays, and mtk is its consumer.** That is a better justification than
the hypothetical hand-built user outer `HS-5` was leaning on, and it settles
`V-5` at `V-5a` on evidence rather than on judgement. **`Q-6` is answered
unless the owner disagrees with the reasoning.**

## 15.6 The finding

**The user's surface and mtk's own surface turn out to be the same surface.**
The mailbox and the pool declare an outer, take one alias, and call `create`'s
neighbours exactly as a user does. Nothing in `mtk::inner` is reserved for the
implementation.

That is a stronger argument for `V-6a` than section 13 makes, and it is a
stronger argument for `RT-6` — *the helper is a first-class citizen, the books
lead with it* — than section 3 makes. **A toolkit whose own containers are
ordinary clients of its public helper has no second, privileged way to use
itself.** If the owner rejects `V-6a`, this paragraph is what is being given
up.
...
**Section 9 by number, last by position:** it was written before sections 10–15
