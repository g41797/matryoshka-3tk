...
**Version 007, superseding
[3tk-example-rules-006.md](backup/3tk-example-rules-006.md)
(now in this repo's `backup/`).** Written by `3TK-83`, 2026-09-15, on the
owner's ruling of the same day. Two changes from `006`, and nothing else:

- **New: *A comment is written for the docs reader — MUST*.** No catalog, entry
  number, file number or port history in an example's comments.
  - A group page lists its examples by module path.
  - A rule with no code of its own is stated as a fact on the group page.
- **Measured: a backticked module path links.** The auto-link paragraph now
  says so.

`006`'s change from `005`: the promise this
...
- **A backticked `Module::symbol` auto-links, if it resolves.** Measured
  ...
  capitalized forms are guaranteed to link. **A module path links too.**
  Measured by `3TK-83`, 2026-09-15: `shc::a_slot_and_transfer`'s page,
  rendered in headless Chrome, links all four module paths in its description.

**Every diagram is fenced.** ` ``` ` on its own line, the diagram, ` ``` ` on
its own line. Unfenced box drawing collapses.

## A comment is written for the docs reader — MUST

**Ruled by the owner, 2026-09-15, and applied by `3TK-83`.**

The reader of an example's page has the generated docs and the source.
The reader has never seen the catalog.

- **No catalog reference in a comment.** No *catalog*, *entry N*, or *row*.
- **No file number in a comment.** Not `NNN-name.c3`, not `012`.
  - The number stays in the filename. It still names the catalog row.
- **No port history in a comment.** Not *new in 3tk*, not *ztk cannot*.
  - A difference from another port goes in `3tk-port-findings-005.md`.
- **Another example is named by its module path**, in backticks.
  - `` `shc::a_slot_and_transfer::empty_slot` ``, never `001-empty_slot.c3`.
  - A backticked module path links to that module's page.
- **A group page is a short intro, then one bullet per example.**
  - The bullet is the module path, then a short hook.
- **A rule with no code of its own is a fact on the group page.**
  - *"A `Slot` is never overwritten."*, not *"Entry 2 has no file."*
- **No sentence about the page itself.** Not *"This module holds no code of its
  own."* The page shows that.

**What goes red.** Nothing automatic. The stage that writes a comment runs
this over `examples/*.c3`, and every hit is a defect:

```
grep -nE 'catalog|[Ee]ntr(y|ies) [0-9]|[0-9]{3}-[a-z_]+\.c3|`[0-9]{3}`|\b(ztk|otk|dtk)\b|new in 3tk'
```

- The file-number part catches `001-empty_slot.c3`.
- The bare-number part catches `` `018` ``.
- The port part catches *ztk cannot*.
- The group name `k_new_in_3tk` is an identifier, so the check does not match it.
