...
[3tk-example-rules-007.md](3tk-example-rules-007.md), and a rule is changed
there and nowhere else.

**Version 005, by `3TK-88`, 2026-09-17.** Entry 17 dispatches by `is`.

- `OuterHelper.is` is new: a yes/no answer for a `Slot*`, an `Inner*` or an `any*`.
- Entry 17's branches release the outer and never read the pointer, so `look` became `is`.
- Ruled by the owner in `3tk-any-border-001.md`.
- `004` is in this repo's `backup/`.
- **No other pattern changed.**

**Version 004, superseding `3tk-patterns-004.md` (now in this repo's
`backup/`).** The only change from `003`: 3tk has exactly two terms:
**`Inner`**, a real C3 type and the field you lend, and **`Outer`**, a role
...
`003` was the only change from `002`: 3TK-59 removed the `alias Handle =
Inner*`, so every code shape spells the pointer `Inner*`.
...
**The word is Outer.** Never `Outer`. **The one exception is a ztk entry
title quoted as a title** — *Cancellation keeps the outer where it is* is that
...
| 14 | [Stack outers are illegal](#14--stack-outers-are-illegal) | not carried over — the owner's ruling |
...
| 59 | [The optional-declaration walk](#59--the-optional-declaration-walk) | **new** — `while (Inner* inner = ...)` |
...
- `fill` on a full Slot aborts in a checking build, and the abort names the
  Slot.
- A Slot holds one inner. Overwriting one loses the outer it held with no
...
- `push_back` takes an `Inner*`, so it cannot empty a Slot. Every call site
...
**Do not** reach for it for a stack outer — there is no such thing as a legal
stack outer to reach for. Entry 14.
...
- For an outer that carries no `Allocator` field, `mtk::helper::init` on a
  heap-allocated pointer is the call, and the release is yours. **Never on a
  stack instance.** Entry 14.
...
- One field, sixteen bytes, and your struct is the inner. No wrapper, no
...
*Ruled, not carried over.* ztk allowed a stack outer for the crossing demo
alone, and 3tk's own catalog once did too. **This version retires that
pattern. The owner ruled it illegal**, not only where it crosses a mailbox or
a thread — everywhere.

**Why it cannot be permitted, not even for a demo.**

- 3tk computes an outer's address from its embedded `Inner` at every
  crossing — a `fieldParentPtr`-style offset from a fixed field. That address
  has to stay valid for as long as any inner, queue entry, or mailbox
  reference to the outer can still be reached.
- A stack struct's address is valid for exactly one lexical instance of that
  one frame. A copy of the struct — a value return, a pass-by-value, a plain
  `Event e2 = e;` — puts a second `Event` at a different address while any
  inner taken earlier still points at the first. A use after the frame
  returns reaches the same way, into memory that is no longer the outer at
  all.
- **It can appear to work.** Nothing catches the mistake at the point it is
  made. The fault surfaces later, unpredictably, wherever the stale address is
  next read — the shape a bug report calls "sometimes it fails."
...
**There is no code shape for this entry.** The prohibition is the entry, and
no example in this catalog or in `3tk/examples/` creates an outer on the
stack.
...
- `pop_front` gives an `Inner*`, not an inner, and resets it before it returns.
  **A popped inner is never linked**, so it drops straight into a Slot or into
  `send`.
...
**When to use.** You hold the outer, and there are two or three identities.
...
- `is` answers the identity question, and `false` is a normal answer, so the calls chain.
- No branch reads the pointer, so no branch crosses.
- A branch that wants the typed pointer uses `look` instead: null on a mismatch.
- `is` takes an `any*` too. An `any` that is not an outer, an io event, answers `false`.
...
- The chain has no final branch here, and that is deliberate: the pool refuses
an identity
...
- The receiver reaches the endpoint through the field. `w.mbx` is a real
  `Mailbox*`, not an inner to convert.
...
- `mailbox::to_inner` crosses out and `mailbox::of` crosses back, the same as
...
- **The instance check is a real pointer comparison.** Both sides are
  `Mailbox*`. Under an inner-only API it compared two look-alike inners, and
...
- A pool is an outer, so it crosses exactly as a mailbox does:
  `pool::to_inner`, `pool::of`, `pool::TYPE`.
...
- `pop_front` resets the inner before it returns it, so it drops straight into
...
the outer where it is* and *Close versus Cancel* have nothing to land on: there
...
2. Close the mailbox the workers receive from. **That is the end-of-stream
...
[3tk-example-rules-007.md](3tk-example-rules-007.md).
...
```c3
struct Ticket { Inner inner; Allocator alloc; int seq; }
...
back an `Inner*` and uses null to mean *no more*.
...
{
    usz n;
    while (Inner* inner = q.pop_front()) { n++; }
    return n;
}
```
...
- **The declaration is the condition.** The inner is in scope inside the body
...
| Graceful cancel walk — recover in-flight outers | `Io.Select`, and cancellation |
| cancelDiscard — timer-only or no-outer sources | `Io.Select`, and cancellation |
...
| Cancellation keeps the outer where it is | the same |
...
- **It does not restate the reference.** A pattern is an assembly of the
  surface; `3tk-api-008.md` is the verification table.
