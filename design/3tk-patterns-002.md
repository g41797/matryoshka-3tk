...
[3tk-patterns-001.md](https://github.com/g41797/matryoshka-tk/blob/main/design/secondary/lang/c3/backup/3tk-patterns-001.md)
(now in `matryoshka-tk`'s `backup/`).** The only change from `001`: entry 14
is rewritten. The owner reviewed it and ruled a stack outer illegal, not only
across a mailbox or thread boundary — see the entry for why. Nothing else in
this file changed. **This version, and every later one, lives in
