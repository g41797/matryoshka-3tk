...
**Version 003, superseding `3tk-patterns-002.md` (now in this repo's
`backup/`).** The only change from `002`: the `Inner*` alias is gone from
3tk, so every code shape spells the handle `Inner*`. 3TK-59 removed it — a
concept gets a C3 type when the type system can express useful semantics for
it, and an `alias` expresses none. **The word *handle* is kept**, here and in
the prose of every port's books, because it names a role. No pattern changed.
...
**Written by 3TK-49**, from `../../matryoshka-tk/design/secondary/lang/c3/3tk-staging-plan-019.md`.
...
| 59 | [The optional-declaration walk](#59--the-optional-declaration-walk) | **new** — `while (Inner* h = ...)` |
...
```c3
fn void p12_type_crossing(Inner* h)
{
...
**When to use.** You hold a handle — an `Inner*` — and want the typed pointer back.

**Code shape.**

```c3
fn void p13_recover(Inner* h, Slot* s)
{
...
- 3tk computes an outer's address from its embedded `Inner` at every
  ...
  has to stay valid for as long as any handle, queue entry, or mailbox
...
- A stack struct's address is valid for exactly one lexical instance of that
  ...
  handle taken earlier still points at the first. A use after the frame
...
- `pop_front` gives an `Inner*`, not a node, and resets it before it returns.
...
```c3
fn void p19_switch_dispatch(Inner* h)
{
    switch (h.link.type)
    {
        case Event::typeid:
...
```c3
fn void p60_guarded(InnerQueue* q, Inner* h)
{
...
- **It does not restate the reference.** A pattern is an assembly of the
  surface; `3tk-api-002.md` is the surface.
