...
**Written by `3TK-67`, 2026-09-08. This is `005`, by `3TK-74`, 2026-09-09**,
which added **Rule 7 — every outer the helper creates declares both hooks** —
and rewrote Rule 4's last paragraph, whose premise Rule 7 deletes. Nothing else
changed: Rules 1–6 are `004`'s, word for word, and so are the stage rules.

**The stage rules moved by one, and a citation to an older number resolves by
adding one.** What `004` numbered 7–12 this file numbers 8–13, because the new
rule belongs in Part 1 and Part 1 ends at Rule 7. `003` did the same when it
added Rule 6. **Live text written against `004` is not rewritten for this** —
`3tk-staging-plan-035.md`'s *Rule 9 governs the order* is `004`'s 9 and this
file's 10, and the log's citations were correct when they were written.

`004` is by `3TK-72`, which added the document-versioning rule. `003` is by
`3TK-71`, which added Rule 6 — the test trees allocate their outers. `004`,
...
## 8. Before and after every stage: compact, clear, or nothing
...
## 9. Before every stage: which model is suitable, and why
...
## 10. The exemplar comes before the sweep
...
## 11. A stage tunes the scripts and CI, and the port is one line
...
## 12. Fix the definition; do not halt on it
...
## 13. A document is versioned, not asked about
