...
**This is 007.** It carries all of `006` and removes the `Handle` alias. A
concept gets a C3 type when the type system can express useful semantics for
it; otherwise it stays vocabulary. `Slot` earns a type. `Handle` did not — it
was an `alias`, so the compiler only ever saw `Inner*` — and the source now
says `Inner*`. **The word *handle* stays**, here and in every port's books,
because it names a role and the porting rule leaves each port to spell it.
See 3TK-59.

`006`, by 3TK-58, made `Mailbox` and `Pool` opaque handles — `typedef ... =
...
see [3tk-patterns-003.md](3tk-patterns-003.md) entry 14. The walkthrough now
...
- `Inner*` — a pointer to an embedded `Inner`. One item, with the type
  forgotten. It is what the books call a **handle**, and 3tk transports
  nothing else. *Handle* is a word in these books and not a type in the
  source: the pointer is the whole of it, so the source spells it as what it
  is.
...
2. **Transport.**
   ...
   - `mtk::managed::create` allocates on the heap, writes the identity, and
     ...
     [3tk-patterns-003.md](3tk-patterns-003.md) entry 14.
...
macro bool is_mine(Inner* h, $Type)
```
...
macro must_from_handle(Inner* h, $Type)
```

- `to_handle` — from your pointer to the handle, an `Inner*`. Null in, null out.
- `from_handle` — from an `Inner*` back to `$Type*`.
...
fn void   Slot.fill(&self, Inner* h)
```
...
fn bool   is_linked(Inner* h)
fn void   reset(Inner* h)
```
...
- `Mailbox` — the tool itself, as an opaque handle following the idiom C3's
  ...
  layout, not whether a field exists at all. Every method casts the handle
...
```c3
macro Inner* to_handle(Mailbox* p)
macro Mailbox* of(Inner* h)
```

- `to_handle` — from a `Mailbox*` to its handle, an `Inner*`. Cannot fail.
...
- `Pool` — the tool itself, as an opaque handle following the same idiom as
  ...
  Every method casts the handle back to `_Pool*` as its first line. Created on
...
```c3
macro Inner* to_handle(Pool* p)
macro Pool* of(Inner* h)
```

- `to_handle` — from a `Pool*` to its handle, an `Inner*`. Cannot fail.
- `of` — the checking crossing back. Null when the handle names another type.
...
The identity sits in the same field as the chain link.
The identity says what the outer type is.
A pointer to an embedded `Inner` is one item, with the type forgotten.
That pointer is what the books call a handle, and 3tk transports nothing else.
It is spelled `Inner*`, because that is all it is.
...
`pool.c3`'s module block once opened `while (mtk::Inner* h = ...)`, a spelling
...
outright, and source and reference both write the handle as `Inner*`.
