# 3tk — API verification table, 004
...
**004 replaces 003, revised by 3TK-60 on 2026-09-04**, which applied the
...
`Pool.bucket_for`, `Pool.take_back`, `Pool.take_back_inner`.
...
| `mtk` | `inner.c3` | `Inner`, `Slot`, the faults, `@check` |
| `mtk` | `queue.c3` | `InnerQueue` and its walker |
| `mtk` | `stack.c3` | `InnerStack` |
| `mtk::helper` | `helper.c3` | the crossings between a typed pointer and an inner |
...
- **Contract** — `@param to : "the inner this outer now links to; itself, if
it is the end"` — `inner.c3:24`.

### `fn Inner* Inner.points_to(&self) @inline`

- **What** — the chain link this outer points at.
- **Promises** — null if the outer is on no chain. The last outer of a chain
  points at itself, so `n.points_to() == n` is the end test.
- **Costs** — O(1).

### `fn bool is_linked(Inner* inner) @inline`

- **What** — the link test.
- **Promises** — exact. No false negative and no false positive, for every path
  through the public surface. An outer alone on a chain reports true.
- **Costs** — O(1). No field beyond `Inner.link`, and no walk.

### `fn void reset(Inner* inner) @inline`

- **What** — clears the link after a removal, so the outer can be inserted
  again.
...
- **What** — empty means the outer is elsewhere.
- **Costs** — O(1).

### `fn bool Slot.is_full(&self) @inline`

- **What** — full means the outer is here, and this Slot's holder is
  responsible for it.
- **Costs** — O(1).

### `fn Inner* Slot.peek(&self) @inline`

- **What** — look at the inner without taking it.
- **Promises** — the Slot is unchanged. Null on an empty Slot.
- **Costs** — O(1).

### `fn Inner* Slot.take(&self) @inline`

- **What** — take the inner out and clear the Slot.
- **Promises** — null on an empty Slot, and the Slot stays empty.
- **Costs** — O(1).

### `fn void Slot.fill(&self, Inner* inner) @inline`

- **What** — put an inner in.
- **Promises** — a full Slot is never overwritten.
- **Costs** — O(1).
- **Contract** — `@param h : "the inner to place; must not be null"` —
`inner.c3:144`.
- **Checks** — `@check(h != null, "Slot.fill with a null inner")` — `inner.c3:149`.
- **Checks** — `@check(self.is_empty(), "never overwrite a full Slot")` —
`inner.c3:151`.
...
- **Checks** — `$assert $seen != 0 : "type " +++ $Type::name +++ " has no
Inner field; it cannot be a Matryoshka outer"` — `inner.c3:175`.
- **Checks** — `$assert $seen == 1 : "type " +++ $Type::name +++ " has more
than one Inner field; exactly one is allowed"` — `inner.c3:177`.
...
- **Checks** — `$assert $off >= 0 : "type " +++ $Type::name +++ " has no
Allocator field; use mtk::helper instead of mtk::managed"` — `inner.c3:197`.
...
- **Checks** — `@check(h != null, "insert of a null inner")` — `queue.c3:50`.
...
- **Checks** — `@check(h != null, "insert of a null inner")` — `stack.c3:35`.
- **Checks** — `@check(!is_linked(h), "the outer is already on a chain")` —
`stack.c3:37`.
...
- **What** — add on top.
- **Promises** — an outer pushed onto an empty stack is the bottom and is
  self-linked. Otherwise it points at the outer it covers.
- **Costs** — O(1), plus the guard above.

### `fn Inner* InnerStack.pop(&self)`

- **What** — take from the top.
- **Promises** — null on an empty stack. The returned outer's link is cleared.
- **Costs** — O(1). The sole outer is recognised by `h.points_to() == h`.

---

# `mtk::helper` — helper.c3

**Every crossing between a typed pointer and a type-erased inner happens in
...
line.
    ...
    struct Msg { int id; Inner inner; char[64] body; }
...
- **Promises** — a null inner is not mine. An outer whose `init` was never
...
- **Costs** — O(1), one `any_make`. The type is inferred from the pointer; no
  call site names it.
- **Contract** — `@param outer : "a pointer to the outer"` — `helper.c3:28`.
- **Contract** — `@require $defined($Typeof(*outer)::members) : "not a struct
that embeds an Inner"` — `helper.c3:29`.

## Outbound

### `macro Inner* to_inner(outer)`

- **What** — from a typed pointer to an inner.
...
- **Contract** — `@param outer : "a pointer to the outer, or null"` —
`helper.c3:44`.
- **Contract** — `@require $defined($Typeof(*outer)::members) : "not a struct
that embeds an Inner"` — `helper.c3:45`.

## Inbound, from an inner
...
- **Promises** — a mismatch is a defect of the program. **One check, not two**:
  a null inner and a wrong identity are the same kind of wrong.
- **Costs** — O(1). The `@require` compiles out in a fast build.
- **Contract** — `@param $Type : "the outer type asserted"` — `helper.c3:72`.
- **Contract** — `@require is_mine(h, $Type) : "the inner is not of this
type"` — `helper.c3:73`.
...
- **Costs** — O(1). The returned pointer is computed from the inner `peek`
...
**The crossings are declared on `Inner`, because C3 has no UFCS and a method
cannot be attached to a pointer alias, so a method is reached through an
embedded `Inner` directly.** `to` and `as` are the two names `any` uses for these
operations, so a C3 reader already knows which is which.

### `macro Inner.to(&self, $Type)`

- **What** — the checking crossing, as a method. Forwards to `from_inner`.
- **Costs** — O(1).
...
outer keeps the allocator it was created with, so its release takes none.
...
- **What** — allocate an outer and fill a Slot with it.
- **Promises** — it fills a Slot and does not return a pointer. On an
  ...
  The allocator is written into the outer and kept for life. `helper::init` is
  called.
- **Costs** — one allocation, through `alloc::new_try`. The plain `alloc::new`
  ...
  true on.
- **Contract** — `@param $Type : "the outer type, which must carry an
Allocator field"` — `managed.c3:22`.
- **Contract** — `@param a : "the allocator the outer keeps for life"` —
`managed.c3:23`.
- **Contract** — `@param slot : "an empty Slot, filled on success"` —
`managed.c3:24`.
- **Checks** — `mtk::@check(slot.is_empty(), "an acquisition asserts the Slot
is empty on entry")` — `managed.c3:30`.

### `macro void release($Type, Slot* slot)`

- **What** — free the outer, with the allocator it kept.
...
- **Costs** — one free.
- **Contract** — `@param $Type : "the outer type, which must carry an
Allocator field"` — `managed.c3:44`.
- **Contract** — `@param slot : "the Slot holding the outer; empty
afterwards"` — `managed.c3:45`.
...
- **Checks** — `$assert $off >= 0 : "type " +++ $Type::name +++ " has no
Allocator field; use mtk::helper instead of mtk::managed"` — `managed.c3:74`.
...
- **What** — from a mailbox to an inner. Forwards to `mtk::helper::to_inner`.
...
- **Promises** — **the mailbox must be closed first, and this is the one
  ...
  parameter.
- **Costs** — one free.
- **Checks** — `always_assert(self._closed, "releasing an open mailbox")` —
`mailbox.c3:96`.
...
- **Checks** — `mtk::@check(slot.is_empty(), "an acquisition asserts the Slot
is empty on entry")` — `mailbox.c3:222`.
...
onto it, in receive order"` — `mailbox.c3:265`.
...
it, in receive order"` — `mailbox.c3:311`.
...
the removal. A hint, and stale"` — `pool.c3:49`.
...
flattened across every identity. No order is promised"` — `pool.c3:80`.
...
and no duplicates — both checked"` — `pool.c3:160`.
...
- **Promises** — **the pool must be closed first.** `always_assert`, aborting
  in every build mode: releasing an open pool means the outers it still held
  never reached the close hook.
- **Costs** — two frees.
- **Checks** — `always_assert(self._closed, "releasing an open pool")` —
`pool.c3:215`.
...
mtk::UNKNOWN_IDENTITY` — `pool.c3:253`.
- **Checks** — `mtk::@check(slot.is_empty(), "an acquisition asserts the Slot
is empty on entry")` — `pool.c3:318`.
- **Checks** — `mtk::@check(b != null, "Pool.get for an identity the pool was
not created with")` — `pool.c3:268`.
- **Checks** — `mtk::@check(slot.peek().link.type == want, "the get hook
returned an outer of a different identity")` — `pool.c3:297`.
...
was not created with")` — `pool.c3:327`.
...
not created with")` — `pool.c3:380`.
- **Checks** — `mtk::@check(b != null, "the put hook returned an identity the
pool was not created with")` — `pool.c3:573`, reached through the private
