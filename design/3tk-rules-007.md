...
**Written by `3TK-67`, 2026-09-08. This is `007`, by `3TK-76`, 2026-09-10**,
which corrected ONE list inside Rule 8 and changed nothing else: `overwrite_slot`
is no longer named among the negatives where the bypass IS the violation.
`3TK-76` swept the two test trees against Rule 8 and read that program line by
line — its violation is the second `fill` on a full Slot, and the crossing that
reaches the inner is plumbing beside it. The wrapper aborts at neither, so the
program proves exactly what it proved before. **The other seven names in that
list were each checked the same way and each stayed.** Rule 13 is why this is a
correction in passing rather than a question: the list was written from a plan's
reading and the sweep measured it.

`006` is by `3TK-75`, which added **Rule 8 — white box where the work requires
it, black box everywhere else** — and, under it, the promise the example rules
make about `test/`. Rules 1–7 are `005`'s, word for word, and so are the stage
rules.
...
- **The bypass IS the violation.** A negative that reaches around the helper is
  ...
  `unstamped_inner`, `wrong_type_inner`, `wrong_type_must`,
  ...
  still passes its build. **The test is whether the bypass is what the program
  is about**, not whether a negative happens to contain one: `overwrite_slot`
  was on this list until `3TK-76` read it, and its violation is the second
  `fill` on a full Slot — the crossing beside it is plumbing, and it swept.
